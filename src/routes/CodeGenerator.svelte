<script lang="ts">
	import type { WebauthnAttestation, WebauthnAssertion } from '$lib/utils/webauthn';
	import { buf2hex, buf2base64url, base64url2buf } from '$lib/utils/crypto';

	export let account: WebauthnAttestation;
	export let assertions: WebauthnAssertion[];
	...
	const { r, s, yParity, messageHash, authenticatorData, clientDataJSON } = assertion;
	const clientDataText = new TextDecoder().decode(clientDataJSON);
	const clientData = JSON.parse(clientDataText);
	const clientDataOffset = (substring: string) =>
		clientDataText.indexOf(substring) + substring.length;
...
const assertion = {
  ...
  // flags (binary) = ${authenticatorData[32]?.toString(2) || '0'}
...
fn test_recover_public_key_r1_${i}() {
    ...
    let message_hash: u256 = ${messageHash};
