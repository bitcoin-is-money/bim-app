...
import {Transaction, TransactionHash, TransactionId, TransactionReader} from '@bim/domain/user';
...
describe('TransactionReader', () => {
  ...
  it('returns transactions for account', async () => {
    ...
    const result = await service.fetchForAccount({accountId});
  ...
  it('uses default limit=10 and offset=0', async () => {
    ...
    await service.fetchForAccount({accountId});
